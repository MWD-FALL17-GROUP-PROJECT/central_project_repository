...
def personalizedPageRank(similarityDataFrame,seed,alpha):
    ...
    for node in similarityDataFrame.columns[(similarityDataFrame == 0).all()]:
        similarityDataFrame.loc[node,node] = 1
    ...
    seed = [check(i,seed,nodes) for i in seed]
    ...
    while np.sum(np.abs(Uq-Uqi)) > maxerr:
        Uqi = Uq
        Uq = alpha*(Uq.dot(npMatrix)) + (1-alpha)*Vq
